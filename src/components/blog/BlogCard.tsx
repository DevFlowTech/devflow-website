...
export default function BlogCard({ post }: BlogCardProps) {
  ...
  return (
    <div className="group block bg-devflow-charcoal border border-white/[0.06] rounded-xl overflow-hidden hover:border-devflow-green/30 transition-all duration-300 hover:shadow-lg hover:shadow-devflow-green/5 flex flex-col h-full">
      {/* Image */}
      <Link
        href={postHref}
        className="relative aspect-video w-full overflow-hidden bg-devflow-dark border-b border-white/[0.06] block"
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-3 mb-3">
          <Link
            ...
            className="px-2.5 py-1 bg-devflow-green/10 text-devflow-green text-xs font-medium rounded-full border border-devflow-green/20 hover:border-devflow-green/50 hover:bg-devflow-green/20 transition-colors"
          >
            {post.category}
          </Link>
        ...
        <Link href={postHref} className="mb-3 block">
          <h3 className="text-xl font-bold text-white group-hover:text-devflow-green transition-colors line-clamp-2">
            {post.title}
          </h3>
        </Link>
